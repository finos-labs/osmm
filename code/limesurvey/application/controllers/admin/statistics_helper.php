...
// echo "</pre>";				
 ...
 protected function displayAggrSimpleResults($outputs, $results, $rt, $labels, $outputType='html', $surveyid, $usegraph=1,$cnt)
    {
        ...
        if ($outputType == 'html') {
            ...
            if ($usegraph == 1 && $bShowGraph && get_class(Yii::app()->getController()) !== 'Statistics_userController') {
			   ...
			   // echo "</pre>";
				...
				$statisticsoutput .= Yii::app()->getController()->renderPartial('/admin/export/generatestats/simplestats/_statisticsoutput_aggregate_graphs', $aData, true);
			   }else{
...
//   statisticsoutput         
        ...
        if ($usegraph == 1) {
            ...
            if ($bShowGraph == true) {
                ...
                if ($outputType == 'xls' || $outputType == 'pdf') {
					...
					$cachefilename = createChart($qqid, $qsid, 0, $graphLbl, $gdata, $grawdata, $MyCache, $sLanguage, $outputs['qtype']);
                }
    ...
    public function generate_simple_statistics($surveyid, $allfields, $q2show = 'all', $usegraph = 0, $outputType = 'pdf', $pdfOutput = 'I', $sLanguageCode = null, $browse = true)
    {
		...
		if(incompleteAnsFilterState()!="aggregate"){    //Lotus
        ...
        foreach ($runthrough as $rt) {
			$rcount = $rcount + 1;  
				if($rcount <= 13){
			$aquery .= "AVG(".$rt.")". " G".$rcount.",";
				}
				else{
					$dcount = $dcount + 1 ;
					$aquery.="AVG(".$rt.")"." D".$dcount.",";
				}
			}
			$aquery=substr($aquery, 0,-1); // To Remove last comma(,)
			
			$fquery = "SELECT {$aquery} FROM lime_survey_$surveyid";
			
			$results1 = Yii::app()->db->createCommand($fquery)->queryAll();
			
			
		$allLables = array("Goals,Objectives and Resources","Corporate Alignment","Community and Ecosystem Management","Opeartions","Culture","Lifecycle Management","Risk Management","Compliance and Assurance Management","Security Management","Consumprion","Contribution","Publication","Inner Source","Strategy","Management","Usage");

		$alist = array(array_slice($allLables,0,5), array_slice($allLables,5,4), array_slice($allLables,9,4),array_slice($allLables,13,3));
		
		$arrayData = array(array_slice($results1[0],0,5), array_slice($results1[0],5,4), array_slice($results1[0],9,4),array_slice($results1[0],13,3));
		// echo "<pre>";
		// print_r($alist);
		// print_r($arrayData);
		// echo "</pre>";				
		...
		foreach($arrayData as $asData){
			...
			$display = $this->displayAggrSimpleResults($outputs,$asData, $rt,$alist[$cnt], $outputType='html', $surveyid, $usegraph,$cnt);			
			$cnt = $cnt + 1;
			// if ($count % 2 == 0) {
			// //$sOutputHTML .= '<div class="clearfix visible-lg-block"></div>';
			// }
